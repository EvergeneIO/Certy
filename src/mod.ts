...
for await (const event of watcher) {
  ...
  if (!configs.SQLITE) {
    ...
    for (const [domain, location] of Object.entries(data)) {
      const res = (
        await database.con.query(
          `SELECT * FROM proxy_host WHERE (domain_names LIKE '%"${domain}"%') AND is_deleted = 0`
        )
      )[0];
    ...
    for (const [domain, location] of Object.entries(data)) {
      const res = [
        ...db
          .query(`SELECT * FROM proxy_host WHERE (domain_names LIKE '%"${domain}"%') AND is_deleted = 0`)
          .asObjects(),
      ][0];
      await copyCerts(domain, location, res);
    }
