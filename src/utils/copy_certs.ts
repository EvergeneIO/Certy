...
export function copyCerts(domain: string, location: Service | null, res: any) {
  ...
  for (const [cert, name] of Object.entries(certs)) {
    if (!existsSync(`./data/${dir}`)) Deno.mkdirSync(`./data/${dir}`);
    copySync(`./letsencrypt/live/npm-${res.certificate_id}/${cert}.pem`, `./data/${dir}/${name}.pem`, { overwrite: true });
    console.log(`Copied Cert from ./letsencrypt/live/npm-${res.certificate_id}/${cert}.pem to ./data/${dir}/${name}.pem`)
  }
}
